    ...
    if Path::new(r1_path).exists() && Path::new(r2_path).exists() {
        ...
        for pair_result in paired_reader.into_paired_records().strict_pairing(true) {
            ...
            if count == 1 {
                ...
                println!("  R1: {} ({}bp)", String::from_utf8_lossy(&r1.id), r1.seq.len());
                println!("  R2: {} ({}bp)", String::from_utf8_lossy(&r2.id), r2.seq.len());
            }
        }
...
fn filtering_example() -> Result<()> {
    ...
    let _quality_filter = QualityFilter::new()
        .min_quality(20.0)
        .min_length(50);
...
fn barcode_example() -> Result<()> {
    ...
    let demux = Demultiplexer::new(config.clone(), barcodes)
        .error_correction(true);
    
    println!("\nDemultiplexing simulation:");
    let test_barcodes = vec![
        b"ATCGATCG",
        b"GCTAGCTA",
        b"ATCGATGG",
        b"TTTTTTTT",
...
fn metrics_example() -> Result<()> {
    ...
    if Path::new(test_file).exists() {
        ...
        println!("  Read length range: {}-{}", summary.min_length, summary.max_length);
        ...
        if pos_stats.len() > 0 {
            ...
            for stat in pos_stats.iter().take(5) {
                println!("  Position {}: mean={:.1}, median={}, Q25={}, Q75={}",
                         stat.position + 1, stat.mean, stat.median, stat.q25, stat.q75);
            }
        }
