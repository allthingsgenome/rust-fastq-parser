use fastq_parser::{
    FastqReader, QualityFilter, AdapterTrimmer, FilterStats,
    parallel::{ParallelProcessor, ParallelFilterProcessor},
...
fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        ...
        eprintln!("  {} input.fastq.gz --filter --trim --output filtered.fastq", args[0]);
...
fn process_simple(input_path: &str) -> Result<()> {
    ...
    println!("Average read length: {:.2}", total_length as f64 / count as f64);
...
fn print_statistics(input_path: &str) -> Result<()> {
    ...
    println!("  Average read length: {:.2}", total_bases as f64 / total_records as f64);
    println!("  GC content: {:.2}%", (gc_count as f64 / total_bases as f64) * 100.0);
    println!("  Quality encoding: {:?}", quality_encoding.unwrap_or(QualityEncoding::Unknown));
    println!("  Average quality score: {:.2}", total_quality / total_records as f64);
